...
### 阶段 2 实验记录：队列 timeout 三组 PUT 测试

实验目标：

```text
观察队列已满时，osMessageQueuePut() 在不同 timeout 参数下的行为：
1. timeout=2000：等待队列出现空间后发送成功；
2. timeout=500：等待时间不足，最终超时；
3. timeout=0：不等待，立即返回失败。
```
...
队列长度为 1。生产者先发送 `put-1` 填满队列，再发送 `put-2`；消费者延时 1000 tick 后取出第一条消息。

串口日志字段：

```text
op      操作类型，put-1 表示第一次发送，put-2 表示第二次发送，get 表示接收。
seq     消息序号。
status  队列 API 返回值。
elapsed 实际等待时间，单位为 tick。
count   当前队列中的消息数量。
space   当前队列剩余空间。
```

#### 测试一：timeout=2000，等待后发送成功
...
put-1 立即成功，队列变满。
put-2 因队列已满而阻塞等待。
消费者约 1000 tick 后取走第一条消息，队列出现空间。
put-2 被唤醒并成功发送第二条消息。
因此 put-2 的 elapsed 约为 1000 tick，而不是 0。
```

注意：`get` 日志中仍可能显示 `count=1 space=0`，因为消费者取走第一条消息后，生产者立即补入了第二条消息。

#### 测试二：timeout=500，等待后超时
...
```text
put-2 最多等待 500 tick。
消费者要到约 1000 tick 后才取消息。
500 tick 等待结束时队列仍然是满的，所以 put-2 返回 osErrorTimeout。
第二条消息没有进入队列，第一条消息仍然保留。
消费者之后立即取出第一条消息，队列变为空。
```
...
```text
put-2 发现队列已满后不等待，立即返回 osErrorResource。
第二条消息没有进入队列。
队列中的第一条消息不会丢失，消费者稍后仍然可以正常取出。
```

三组实验对比：

| `timeout` | 队列满时行为 | 返回值 | 是否阻塞 |
| ---: | --- | --- | --- |
| `0` | 立即失败 | `osErrorResource` | 否 |
| `500` | 等待 500 tick 后失败 | `osErrorTimeout` | 是 |
| `2000` | 等待期间出现空间则成功 | `osOK` | 是 |

实验结论：

```text
timeout 决定任务在队列资源不可用时是否等待，以及最多等待多久。
timeout=0 适合不允许阻塞的场景，但必须处理发送失败。
有限 timeout 可以在等待和实时性之间取得平衡。
如果业务允许一直等待，可以使用 osWaitForever，但要注意任务可能长期阻塞。
```

### 阶段 2 实验记录：队列 timeout 三组 GET 测试

实验目标：

```text
观察队列为空时，osMessageQueueGet() 在不同 timeout 参数下的行为：
1. timeout=0：不等待，立即返回资源不可用；
2. timeout=500：等待一段时间后仍无消息，返回超时；
3. timeout=2000：等待期间收到消息，接收成功。
```
...
生产者启动后延时 1000 tick，再向队列发送一条消息。消费者每轮尝试接收消息，接收完成后延时约 3000 tick。

#### 测试一：timeout=0，队列空时立即失败
...
```text
第一次 get 执行时队列为空，timeout=0，因此立即返回 osErrorResource。
生产者约 1000 tick 后发送消息，消息进入队列。
消费者下一轮接收时，队列中已有消息，因此立即返回 osOK。
```

#### 测试二：timeout=500，等待后超时或成功
...
```text
消费者最多等待 500 tick。
生产者要到约 1000 tick 后才发送，超过了消费者的等待上限。
因此第一次 get 返回 osErrorTimeout。
生产者随后发送成功，消息保留在队列中。
消费者下一轮可以立即取出该消息。
```

如果消费者开始等待时，队列中的消息在 500 tick 内到达，则 get 会返回 osOK；否则返回 osErrorTimeout。

#### 测试三：timeout=2000，等待消息后成功
...
```text
消费者先发现队列为空，于是阻塞等待。
生产者约 1000 tick 后发送消息，消费者被唤醒。
get 等待约 1000 tick 后成功返回，而不是等待完整的 2000 tick。
消息取出后，队列重新变为空。
```

三组 GET 实验对比：

| `timeout` | 队列空时行为 | 返回值 | 是否阻塞 |
| ---: | --- | --- | --- |
| `0` | 立即失败 | `osErrorResource` | 否 |
| `500` | 等待 500 tick 后仍无消息则失败 | `osErrorTimeout` | 是 |
| `2000` | 等待期间收到消息则成功 | `osOK` | 是 |

GET 实验结论：

```text
timeout=0 时，消费者只检查一次队列，不会阻塞。
有限 timeout 时，消费者会进入 Blocked 状态，直到收到消息或等待超时。
如果消息在 timeout 到期前到达，消费者会被唤醒并返回 osOK。
count=0、space=1 表示消息已被取出，队列当前为空。
count=1、space=0 表示生产者发送成功后，队列当前已满。
```
