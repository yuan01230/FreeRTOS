...
static const uint8_t *LCD_FindTemplateChineseGlyph(const uint8_t *utf8, uint16_t consumed)
{
    uint16_t i;

    if ((utf8 == NULL) || (consumed < 3U))
    {
        return NULL;
    }

    for (i = 0U; i < TFTLCD_ChineseCount; i++)
    {
        if ((TFTLCD_ChineseIndex[i][0] == utf8[0]) &&
            (TFTLCD_ChineseIndex[i][1] == utf8[1]) &&
            (TFTLCD_ChineseIndex[i][2] == utf8[2]))
        {
            return TFTLCD_ChineseFont16x16[i];
        }
    }
...
static uint8_t LCD_ReadFlashChineseGlyph(uint32_t codepoint, uint8_t *glyph)
{
    WCHAR oem;
    uint8_t gb2312[2];
    FontCodecToken token;

    if ((glyph == NULL) || (codepoint > 0xFFFFUL) || (FontStorage_IsReady() == 0U))
    {
        return 0U;
    }

    oem = ff_convert((WCHAR)codepoint, 0U);
    if ((oem < 0xA1A1U) || (oem > 0xFEFEU))
    {
        return 0U;
    }

    gb2312[0] = (uint8_t)(oem >> 8);
    gb2312[1] = (uint8_t)oem;
    if (FontCodec_ParseGB2312(gb2312, &token) != FONT_CODEC_OK)
    {
        return 0U;
    }

    return (FontStorage_ReadGlyph16(token.glyph_offset, glyph) == FONT_STORAGE_OK) ? 1U : 0U;
}

static void LCD_DrawGlyph16x16MsbFirst(uint16_t x, uint16_t y, const uint8_t *glyph)
{
    uint16_t row;
    uint16_t col;
    ...
    for (row = 0U; row < 16U; row++)
    {
        uint16_t bits = ((uint16_t)glyph[row * 2U] << 8) |
                        (uint16_t)glyph[(row * 2U) + 1U];
        for (col = 0U; col < 16U; col++)
        {
            uint16_t color = ((bits & (1U << (15U - col))) != 0U) ? FRONT_COLOR : BACK_COLOR;
            LCD_DrawFRONT_COLOR((u16)(x + col), (u16)(y + row), color);
        }
    }
}

static void LCD_DrawGlyph16x16LsbFirst(uint16_t x, uint16_t y, const uint8_t *glyph)
{
    uint16_t row;
    uint16_t col;
    ...
    for (row = 0U; row < 16U; row++)
    {
        uint16_t bits = (uint16_t)glyph[row * 2U] |
                        ((uint16_t)glyph[(row * 2U) + 1U] << 8);
        for (col = 0U; col < 16U; col++)
        {
            uint16_t color = ((bits & (1U << col)) != 0U) ? FRONT_COLOR : BACK_COLOR;
            LCD_DrawFRONT_COLOR((u16)(x + col), (u16)(y + row), color);
        }
    }
}
...
{
    LCD_DrawLine_Color(x, y, (u16)(x + 15U), y, RED);
    LCD_DrawLine_Color(x, (u16)(y + 15U), (u16)(x + 15U), (u16)(y + 15U), RED);
    LCD_DrawLine_Color(x, y, x, (u16)(y + 15U), RED);
    LCD_DrawLine_Color((u16)(x + 15U), y, (u16)(x + 15U), (u16)(y + 15U), RED);
    LCD_DrawLine_Color((u16)(x + 3U), (u16)(y + 3U), (u16)(x + 12U), (u16)(y + 12U), RED);
    LCD_DrawLine_Color((u16)(x + 12U), (u16)(y + 3U), (u16)(x + 3U), (u16)(y + 12U), RED);
}
...
static uint8_t LCD_Utf8DecodeCodepoint(const uint8_t *in, uint16_t in_len, uint16_t *consumed, uint32_t *codepoint)
{
    ...
    if (((c0 & 0xE0U) == 0xC0U) && (in_len >= 2U) && ((in[1] & 0xC0U) == 0x80U))
    {
        *consumed = 2U;
        *codepoint = ((uint32_t)(c0 & 0x1FU) << 6) | (uint32_t)(in[1] & 0x3FU);
        return 1U;
    }

    if (((c0 & 0xF0U) == 0xE0U) && (in_len >= 3U) &&
        ((in[1] & 0xC0U) == 0x80U) && ((in[2] & 0xC0U) == 0x80U))
    {
        *consumed = 3U;
...
void LCD_ShowTextMixed(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t *text)
{
    ...
    text_len = (uint16_t)strlen((const char *)text);
    while ((offset < text_len) && ((uint16_t)(y + 16U) <= y_end))
    {
        ...
        if (LCD_Utf8DecodeCodepoint(&text[offset], (uint16_t)(text_len - offset), &consumed, &codepoint) == 0U)
        {
            codepoint = '?';
            consumed = 1U;
        }
        ...
        if (codepoint < 0x80U)
        {
            if ((uint16_t)(x + 8U) > x_end)
            {
                x = x0;
                y = (uint16_t)(y + 16U);
                continue;
            }
            LCD_ShowChar(x, y, (u8)codepoint, 16U, 0U);
            x = (uint16_t)(x + 8U);
        }
        else
        {
            const uint8_t *glyph = LCD_FindTemplateChineseGlyph(&text[offset], consumed);
            uint8_t flash_glyph[32];

            if ((uint16_t)(x + 16U) > x_end)
            {
                x = x0;
                y = (uint16_t)(y + 16U);
                continue;
            ...
            if (LCD_ReadFlashChineseGlyph(codepoint, flash_glyph) != 0U)
            {
                LCD_DrawGlyph16x16MsbFirst(x, y, flash_glyph);
            }
            else if (glyph != NULL)
            {
                LCD_DrawGlyph16x16LsbFirst(x, y, glyph);
            }
            else
            {
...
{
    LCD_ShowTextMixed(x, y, (uint16_t)(tftlcd_data.width - x), 16U, (const uint8_t *)str);
}

void LCD_ShowTextUtf8(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const char *utf8)
{
    LCD_ShowTextMixed(x, y, width, height, (const uint8_t *)utf8);
}

void LCD_ShowFontHZ(u16 x, u16 y, u8 *cn)
{
    LCD_ShowTextMixed(x, y, (uint16_t)(tftlcd_data.width - x), 16U, cn);
}
